...
usage()
{
    fprintf(stdout, "setlock: usage: setlock [ -nNxX ] file program [ arg ... ]\n");
    exit(100);
}

int
main(int argc, char *const *argv, const char *const *envp)
{
    int opt;
    int fd;
    const char *file;
    ...
    if ((fd = open(file, O_WRONLY | O_NDELAY | O_APPEND | O_CREAT, 0600)) == -1)
    {
        if (flagx) exit(0);
    ...
    fprintf(stderr, "setlock: fatal: unable to run %s: \n", *argv);
    exit(111);
